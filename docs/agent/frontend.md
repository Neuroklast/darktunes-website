...
| SOS Excel export | `ExcelExportDialog` — body `max-h-[70vh] overflow-y-auto` + `data-lenis-prevent`; column presets on workspace, not localStorage; Raw is a sheet-only toggle (no column picker) |
