...
### Added
- **Multi-tenant SaaS foundation (`feat/multi-tenant-saas`, PR #417 patterns):** Org schema (`organizations`, plans, subscriptions, custom domains, audit log, Stripe event idempotency) + `organization_id` on core CMS tables (default Org #0 = darkTunes). Host resolve + `proxy` headers; public catalog DAL scoped by org; `/pricing` + `/onboarding`; Stripe checkout/webhook; admin Organizations UI + Partner API modules. Cache/R2 helpers. Agent SSOT: [`docs/agent/multi-tenant.md`](docs/agent/multi-tenant.md). Apply `reset.sql` on staging before enabling multi-host pilots.
