...
### Added
- **Multi-tenant SaaS foundation (`feat/multi-tenant-saas`, PR #417 patterns):** Org schema + host proxy; per-label CMS/settings/flags/Sales Statements/media/mailbox/press; staff RLS (org + artist helpers) including tour planner; **journalist_downloads** + **api_credentials** (`label_id`) org-gated; Stripe checkout membership + **`processStripeWebhookEvent`** unit coverage; R2 dual-read; `check:organization-scope` CI gate; agent SSOT [`docs/agent/multi-tenant.md`](docs/agent/multi-tenant.md). Apply `reset.sql` on staging. User-facing name is **Sales Statement**.
