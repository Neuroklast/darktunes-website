...
### Added
- **Multi-tenant SaaS program (Phase 0):** Agent SSOT [`docs/agent/multi-tenant.md`](docs/agent/multi-tenant.md); org host helpers under `src/lib/organizations/` (ported patterns from PR #417); env placeholders for platform domain + Stripe; PRD SaaS surfaces. Work lives on branch `feat/multi-tenant-saas`. No production behavior change yet.
