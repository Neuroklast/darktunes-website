...
### Added
- **Multi-tenant SaaS foundation (`feat/multi-tenant-saas`, PR #417 patterns):** Org schema + `organization_id` on core CMS tables (Org #0 = darkTunes). Host resolve + proxy headers; DAL scoped for public catalog, artists, genres, assets, submissions, sync queue, admin SOS list; portal membership checks host org; isolation unit + e2e specs; `/pricing` + `/onboarding`; Stripe; admin Organizations + Partner API; org branding CSS. Agent SSOT: [`docs/agent/multi-tenant.md`](docs/agent/multi-tenant.md). Apply `reset.sql` on staging before multi-host pilots.
