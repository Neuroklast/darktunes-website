...
### Added
- **Multi-tenant SaaS foundation (`feat/multi-tenant-saas`, PR #417 patterns):** Org schema + host proxy; per-label CMS/settings/flags/Sales Statements/media/mailbox; staff RLS (org + artist helpers) including **tour planner**; **promo_tracks / journalist_applications / accreditation_requests** org-scoped; promo journalist read requires approved application for that org; press-kit staff via assets; interview_requests staff nested; R2 dual-read; Stripe checkout membership gate; `check:organization-scope` CI gate; agent SSOT [`docs/agent/multi-tenant.md`](docs/agent/multi-tenant.md). Apply `reset.sql` on staging. User-facing name is **Sales Statement**.
