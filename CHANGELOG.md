...
### Added
- **Multi-tenant SaaS foundation (`feat/multi-tenant-saas`, PR #417 patterns):** Org schema + host proxy; public/admin CMS + Sales Statements + File Explorer/upload/folders/EPK + **catalog storage stats + portal feedback admin** scoped by org; `check:organization-scope` CI gate; credentials/sync/messages/press/assets; agent SSOT [`docs/agent/multi-tenant.md`](docs/agent/multi-tenant.md). Apply `reset.sql` on staging before multi-host pilots.
