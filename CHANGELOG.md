...
### Added
- **Multi-tenant SaaS foundation (`feat/multi-tenant-saas`, PR #417 patterns):** Org schema + host proxy; per-label CMS/settings/flags/Sales Statements/media/mailbox/press/submissions; staff RLS (org + artist helpers); **financial_audit_events** + **apify_usage_months** (composite PK) org-scoped; Apify Spotify sync host-budgeted; org SaaS/video/feedback/mailbox notes; Stripe checkout + webhook unit coverage; R2 dual-read; `check:organization-scope` CI gate; agent SSOT [`docs/agent/multi-tenant.md`](docs/agent/multi-tenant.md). Apply `reset.sql` on staging. User-facing name is **Sales Statement**.
