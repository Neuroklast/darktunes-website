...
### Fixed
- **SOS Excel download no longer hangs after Confirm:** Original-report workbooks are built in the SOS worker (ExcelJS) and only the xlsx buffer is transferred — Believe files can be huge, so raw rows never clone onto the main thread. A loading toast shows while it builds (up to 5 minutes).
- **Incomplete statements are not downloaded:** If original-report tabs cannot be built, no xlsx is written (ZIP gets a `_EXCEL_NOT_INCLUDED.txt` instead). Summary-only files are named `*_statement_summary-only.xlsx`. Believe commission columns match more header variants. Summary includes a short provenance note.
