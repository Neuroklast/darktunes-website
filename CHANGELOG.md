...
### Added
- **Multi-tenant SaaS foundation (`feat/multi-tenant-saas`, PR #417 patterns):** Org schema + host proxy; per-label CMS/settings/flags/Sales Statements/media; staff RLS (org + **`user_can_access_artist`**) for CMS/assets/sync/finance/metrics/messages/EPK/**tour planner**/promo_log; **message_folders/rules** `organization_id` + DAL; portal admin inbox org-scoped; sync cover-art tenant keys; R2 dual-read; Stripe checkout membership gate; `check:organization-scope` CI gate; agent SSOT [`docs/agent/multi-tenant.md`](docs/agent/multi-tenant.md). Apply `reset.sql` on staging. User-facing name is **Sales Statement**.
