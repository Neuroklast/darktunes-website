...
### Added
- **Artist portal product feedback:** `/portal/feedback` — simple form (category, optional star rating, optional subject, required message) plus own history with status. Admin inbox at `/admin/feedback` (filter by status/category, search, mark reviewed/archive). Table `portal_feedback`; APIs under `/api/portal/feedback` and `/api/admin/feedback`. Distinct from Zammad `/admin/support`.
