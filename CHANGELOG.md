...
### Added
- **Multi-tenant SaaS foundation (`feat/multi-tenant-saas`, PR #417 patterns):** Org schema + host proxy; per-label CMS/settings/portal feature flags/Sales Statements/media; **staff RLS via `user_can_access_organization`** on assets, folders, artists, releases, news, videos, concerts, genres, EPK templates, message templates, site_settings; press/promo host-org; R2 dual-read; platform KV on Org #0; `check:organization-scope` CI gate; agent SSOT [`docs/agent/multi-tenant.md`](docs/agent/multi-tenant.md). Apply `reset.sql` on staging. User-facing name is **Sales Statement**.
