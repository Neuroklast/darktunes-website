...
### Added
- **Multi-tenant SaaS foundation (`feat/multi-tenant-saas`, PR #417 patterns):** Org schema + host proxy; per-label CMS/settings/flags/Sales Statements/media; staff RLS (`user_can_access_organization` + nested **`user_can_access_artist`**) for CMS, assets, sync, Sales Statement/settlement, **invoices/billing/documents/PII**, **metrics**, **label/portal messages**, **EPK**; **sync cover-art tenant keys**; press/promo host-org; R2 dual-read; platform KV on Org #0; Stripe checkout membership gate; `check:organization-scope` CI gate; agent SSOT [`docs/agent/multi-tenant.md`](docs/agent/multi-tenant.md). Apply `reset.sql` on staging. User-facing name is **Sales Statement**.
