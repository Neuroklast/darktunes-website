...
### Added
- **Multi-tenant SaaS foundation (`feat/multi-tenant-saas`, PR #417 patterns):** Org schema + host proxy; **per-label `site_settings`** (brand/legal/theme caches org-keyed) + settlement periods + message templates + Sales Statements + CMS catalog + File Explorer/EPK + storage + feedback; `check:organization-scope` CI gate; agent SSOT [`docs/agent/multi-tenant.md`](docs/agent/multi-tenant.md). Apply `reset.sql` on staging before multi-host pilots. User-facing name is **Sales Statement** (code paths may still use `sos_*`).
