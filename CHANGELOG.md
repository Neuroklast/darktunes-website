...
### Added
- **Multi-tenant SaaS foundation (`feat/multi-tenant-saas`, PR #417 patterns):** Org schema + `organization_id` on core CMS tables (Org #0 = darkTunes). Host resolve + proxy headers; DAL scoped for public catalog, artists, genres, assets, submissions, sync queue; `/pricing` + `/onboarding`; Stripe checkout/webhook; admin Organizations + Partner API; org branding CSS in root layout. Agent SSOT: [`docs/agent/multi-tenant.md`](docs/agent/multi-tenant.md). Apply `reset.sql` on staging before multi-host pilots.
