...
### Added
- **Multi-tenant SaaS foundation (`feat/multi-tenant-saas`, PR #417 patterns):** Org schema + host proxy lookup/suspend gate; public + admin CMS DAL org-scoped; portal + **admin membership gate** on request org; sync queue cron fans out per active org; isolation tests; pricing/onboarding/Stripe; agent SSOT [`docs/agent/multi-tenant.md`](docs/agent/multi-tenant.md). Apply `reset.sql` on staging before multi-host pilots.
