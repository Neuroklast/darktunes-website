...
### Added
- **Multi-tenant SaaS foundation (`feat/multi-tenant-saas`, PR #417 patterns):** Org schema + host proxy; public/admin CMS + **Sales Statements** (history, workspaces, rule presets, period summaries, bronze CSV batches) + File Explorer/upload/folders/EPK + storage stats + feedback; `check:organization-scope` CI gate; credentials/sync/messages/press/assets; agent SSOT [`docs/agent/multi-tenant.md`](docs/agent/multi-tenant.md). Apply `reset.sql` on staging before multi-host pilots. User-facing name is **Sales Statement** (code paths may still use `sos_*`).
