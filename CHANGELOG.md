...
### Added
- **Multi-tenant SaaS foundation (`feat/multi-tenant-saas`, PR #417 patterns):** Org schema + host proxy; per-label CMS/settings/portal feature flags/Sales Statements/media; **staff RLS via `user_can_access_organization`** (assets, folders, EPK templates, message templates, site_settings); press/promo toggles host-org; R2 tenant keys + dual-read; platform KV on Org #0; `check:organization-scope` CI gate; agent SSOT [`docs/agent/multi-tenant.md`](docs/agent/multi-tenant.md). Apply `reset.sql` on staging before multi-host pilots. User-facing name is **Sales Statement**.
