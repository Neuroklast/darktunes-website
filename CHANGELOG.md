...
### Added
- **Multi-tenant SaaS foundation (`feat/multi-tenant-saas`, PR #417 patterns):** Org schema + host proxy; per-label CMS/settings/flags/Sales Statements/media; staff RLS (`user_can_access_organization`) incl. **sync_queue** + **release_submissions**; **sync cover-art tenant keys**; press/promo host-org; R2 dual-read; platform KV on Org #0; `check:organization-scope` CI gate; agent SSOT [`docs/agent/multi-tenant.md`](docs/agent/multi-tenant.md). Apply `reset.sql` on staging. User-facing name is **Sales Statement**.
