...
### Added
- **Multi-tenant SaaS foundation (`feat/multi-tenant-saas`, PR #417 patterns):** Org schema + host proxy; per-label `site_settings` / feature toggles / settlement periods / Sales Statements / media; **R2 tenant keys** (`tenants/{orgId}/…`, Org #0 legacy flat); platform heartbeats/sync lease on Org #0 with composite `site_settings` PK; `check:organization-scope` CI gate; agent SSOT [`docs/agent/multi-tenant.md`](docs/agent/multi-tenant.md). Apply `reset.sql` on staging before multi-host pilots. User-facing name is **Sales Statement** (code paths may still use `sos_*`).
