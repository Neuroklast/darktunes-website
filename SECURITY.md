...
- **Artist multi-tenant isolation (portal):** Membership is via `artist_members` (legacy `artists.user_id` still may exist). Artists only access their own portal rows — RLS enforces this even if the client manipulates requests.
- **Label multi-tenant isolation (SaaS organizations):** Business data is scoped by `organization_id` (Org #0 = darkTunes). Host resolution sets `x-organization-id` in `proxy.ts`; server/DAL code uses `getRequestOrganizationId()` / explicit `organizationId` filters. Staff JWT paths use `user_can_access_organization` / nested `user_can_access_artist` in RLS (not role alone). Platform ops APIs assert membership or `platform_admins` via `assertAdminOrganizationAccess`. Public SELECT for visible CMS content stays host-agnostic at RLS; the app still filters by host org. SSOT: [`docs/agent/multi-tenant.md`](docs/agent/multi-tenant.md).
...
| Org #0 legacy staff free-pass | Transitional single-tenant admin without `organization_users` row | Platform admins + membership preferred; free-pass is Org #0 only (`assertAdminOrganizationAccess`) |
| Multi-tenant schema not applied | Staging/prod lag behind `reset.sql` | Apply multi-tenant block before pilots; see DEPLOYMENT multi-tenant section |
