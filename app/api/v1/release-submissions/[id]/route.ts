...
import { getPartnerReleaseSubmissionById } from '@/lib/partner-api/queries'
...
export const GET = withPartnerAuth(async (req, auth) => {
  ...
  const submission = await getPartnerReleaseSubmissionById(db, auth.organizationId, extractId(req))
