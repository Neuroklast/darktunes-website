...
- **Assets Management**: Folder-based File Explorer / Asset Manager for Cloudflare R2 uploads (single source of truth for all media), with search, bulk selection/delete, folder CRUD, artist assignment, press metadata, bulk press approve/kit actions, inline previews, duplicate detection via SHA-256 hash, a **bucket storage** bar (live R2 listing, not only the catalog), and **Optimize images** on upload (default on).
