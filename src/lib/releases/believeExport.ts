...
export function validateBelieveExport(
  ...
  if (!submission.isrc?.trim()) warnings.push('ISRC is missing — required for most distributor deliveries')
  ...
  if (submission.status !== 'accepted' && submission.status !== 'reviewed') {
    warnings.push(`Submission status is "${submission.status}" — export is intended for reviewed or accepted releases`)
  }
